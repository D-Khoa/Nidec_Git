    ...
    public class PrintItem
    {
        ...
        public bool PrintItems(List<PrintItem> listPrintItem, bool printOneCoppy)
        {
            for (int i = 0; i < listPrintItem.Count; i++)
            {
                if (printOneCoppy)
                {
                    TfPrint.printBarCodeNew(listPrintItem[i].Item_Number, listPrintItem[i].Item_Name, listPrintItem[i].SupplierName, listPrintItem[i].Invoice, listPrintItem[i].Delivery_Date.ToString("yyyy/MM/dd"), listPrintItem[i].Delivery_Qty.ToString(), listPrintItem[i].SupplierCD, listPrintItem[i].OrderNo, listPrintItem[i].Remark);
                }
                else
                {
        ...
        public bool PrintAnItem(PrintItem inItem, bool printOneCoppy)
        {
            if (printOneCoppy)
            {
                TfPrint.printBarCodeNew(inItem.Item_Number, inItem.Item_Name, inItem.SupplierName, inItem.Invoice, inItem.Delivery_Date.ToString("yyyy/MM/dd"), inItem.Delivery_Qty.ToString(), inItem.SupplierCD, inItem.OrderNo, inItem.Remark);
            }
            else
            {
